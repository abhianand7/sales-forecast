...
def model(filename, h, criterion='mse', trees=10):
    ...
    feature_importances_df.to_csv('FeatureImportance-{}.csv'.format(time_stamp), index=False, header=True)
    ...
    #                         if_exists='replace',
    #                         index=False)
    final_data_frame.to_csv('output-{}.csv'.format(time_stamp), index=False, header=True)
    return
